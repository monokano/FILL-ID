...
//
// Glow Id の更新履歴ウィンドウを流用したもの。URL は FILL Id 用に差し替え済み。
// NOTE: GitHub Releases のリポジトリ（monokano/FILL-Id）はまだ存在しない（予定）。
...
class ChangeLogWindowController: NSObject {
    ...
    func show() {
        ...
        if let existing = window, existing.isVisible {
            ...
            existing.orderFrontRegardless()   // InDesign 最前面時でも前面化（共通メニューから開くため）
            return
        }
        ...
        win.orderFrontRegardless()   // InDesign 最前面時でも前面化（共通メニューから開くため）
    }
...
private struct ChangeLogRootView: View {
    ...
    // GitHub Releases（リポジトリは予定。未作成）。
    private let releasesURL = URL(string: "https://github.com/monokano/FILL-Id/releases")!
...
private struct ChangeLogView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> WKWebView {
        ...
        // 日本語は -ja、それ以外は -en の更新履歴 HTML。キャッシュを避けるためクエリに UUID を付与。
        let lang = Locale.current.language.languageCode?.identifier == "ja" ? "ja" : "en"
        let urlStr = "https://tama-san.com/app-page/assets/change-log/fill-id-change-log-\(lang).html?\(UUID().uuidString)"
